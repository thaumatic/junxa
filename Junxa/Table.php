...
class Table
{
    ...
    final public function addDynamicColumn(string $name, $content) : self
    {
        ...
        $columnModel = new $class($this, $name, $res->fetch_field(), null, $alias);
    ...
    final public function row()
    {
        ...
        if ($query->getOption(Junxa::OPTION_SUPPRESS_CACHING)) {
            return $row;
        } else {
    ...
    final public function rows($what = []) : array
    {
        ...
        if ($this->database->getOption(Junxa::DB_CACHE_TABLE_ROWS)
            && $this->primary
            && !$query->getOption(Junxa::OPTION_SUPPRESS_CACHING)
        ) {
